...
def run(args):
    ...
    cv_loader = DataLoader(
        LoaderSelector(args.cv_loader)(args.data.valid,valid_length_func=model.valid_length),
        batch_size=args.experiment.batch_size.cv, shuffle=False)
    tt_loader = DataLoader(
        LoaderSelector(args.ts_loader)(args.data.test ,valid_length_func=model.valid_length),
