...
def clip(audio, thres):
    return clamp(audio, min=-thres,max=thres)

class RandomHardclipAudioset:
    ...
    def __getitem__(self, index):
        for (file, _), examples in zip(self.files, self.num_examples):
            ...
            # hardclip
            thres = 10 ** (random.uniform(-2.0, -0.9)) # includes both ends. about 0.01 (0.5dB) to 0.15 (9dB)
            out = clip(out, thres)
            # end hardclip
