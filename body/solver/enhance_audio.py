...
from ..loader.audio import Audioset, find_audio_files

from ..utils.logging import LogProgress
...
def get_estimate(model, noisy, args):
    ...
    with torch.no_grad():
        ...
        estimate = (1 - args.experiment['dry']) * estimate + args.experiment['dry'] * noisy
    return estimate
...
def enhance(args, model, local_out_dir):
    # Load model
    model.eval()
    out_dir = local_out_dir

    loader = get_dataset(args, model.sample_rate, model.chin)
    loader = torch.utils.data.DataLoader(loader, batch_size=1)

    os.makedirs(out_dir, exist_ok=True)
    logger.info("Enhancing starts now.")
    with ProcessPoolExecutor(args.experiment['num_workers']) as pool:
        ...
        for data in iterator:
            ...
            noisy_signals = noisy_signals.to(args.experiment['device'])
            if args.experiment['device'] == 'cpu' and args.experiment['num_workers'] > 1:
                pendings.append(
        ...
        if pendings:
            ...
            for pending in LogProgress(logger, pendings, updates=5, name="Generate enhanced files"):
                pending.result()
    logger.info("Enhancing ends now.")
